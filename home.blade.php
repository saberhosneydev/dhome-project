        ...
        <div class="card-body">
          ...
          <div class="card">
            <div class="card-header">
              @if($home->sold)
              {{$home->location}}
              <i class="fas fa-check" style="color:lightblue;"></i>
              @else
              {{$home->location}}
              @endif
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST" enctype="multipart/form-data" id="newHome2" action="/homes">
            {{ csrf_field() }}
            <div class="form-group">
              <label for="homelocation">Home Location</label>
              <input type="text" class="form-control {{ $errors->has('location') ? 'animated shake is-danger' : ''}}" name="location" id="homelocation" aria-describedby="homeTitle" placeholder="Enter your home location" value="{{ old('location') }}">
         ...
         <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
          <button type="button" class="btn btn-primary" onclick="document.getElementById('newHome2').submit()">Save changes</button>
        </div>
      </div>
    </div>
  </div>
